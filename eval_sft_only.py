...
model.load_adapter('peft_model_sft_only')
...
for src_lang in LANG_CODES:
    ...
    # save to file
    with open(f'model_outputs/sftonly_{src_lang}_to_{YUE_CODE}.txt', 'w', encoding='utf-8') as f:
        for line in output_lines:
            f.write(f"{line}\n")
    prompts = get_all_prompts(YUE_CODE, src_lang, flores_df)
    output_lines = model_output(model, tokenizer, prompts)
    # save to file
    with open(f'model_outputs/sftonly_{YUE_CODE}_to_{src_lang}.txt', 'w', encoding='utf-8') as f:
        for line in output_lines:
            f.write(f"{line}\n")
