...
def translate_dataset(translator, dataset, src_lang, tgt_lang, current_time):
...
def main():
    ...
    for src_lang in languages:
        translate_dataset(translator, flores_dataset, src_lang, 'Cantonese', current_time)
        # print(f"Translating from {src_lang} to Cantonese")
        # language_data = flores_dataset.get_language_data(src_lang)
        # language_data = language_data[:5]
        # results = [translator.translate(src_lang=src_lang, text=line) for line in tqdm(language_data)]
    ...
    for tgt_lang in languages:
        translate_dataset(translator, flores_dataset, 'Cantonese', tgt_lang, current_time)
        # print(f"Translating from Cantonese to {tgt_lang}")
        # language_data = flores_dataset.get_language_data('Cantonese')
        # language_data = language_data[:5]
        # results = [translator.translate(tgt_lang=tgt_lang, text=line) for line in tqdm(language_data)]
        # path = f"results/{current_time}/Cantonese_to_{tgt_lang}.txt"
        # with open(path, 'w') as f:
        #     for result in results:
        #         f.write(result + '\n')
